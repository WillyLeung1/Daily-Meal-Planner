...
export default class Survey extends Component{
  ...
  getMealPlan = (e) => {
    e.preventDefault();
    const { mealCount, planType, healthPreferences, calories, diet, cookingTime } = this.state;
    ...
    const res = {
      ...
      diet: diet.name,
      cookingTime: cookingTime, // Add cooking time to request data (Jordan)
      meals: meals,
    }
  ...
  render(){
    ...
    const { calories, cookingTime } = this.state; // Extract calories and cookingTime from state (Jordan)
    return(
          ...
          <div className="Survey__content">
            ...
            <form>
              <Tabs defaultIndex={0} ref = {component => {this.tabs = component}} className="Survey__tabs">
                ...
                {/* Add cooking time question (Jordan) */}
                <Tab heading="6">
                  <h2>Estimate Cooking Time (15-999 in minutes)</h2>
                  <input type="number" value={this.state.cookingTime} onChange={this.handleCookingTime}  min="15"/> {/* cooking time input box */}
                  ...
                  {cookingTime < 25 && (
                    <p className="warning">
                      Please note that the preferred cooking time is less than 30 minutes. <br /> 
                      Some meals might not fit within this time while still meeting the calorie 
                      range of {calories.min} - {calories.max} kcal.
                    </p>
                  )}
